...
import { Camera, Square, Play, Music, Loader2, AlertCircle, Key, Activity, Cpu, ScanFace, Info, X, MessageSquare, Volume2, VolumeX } from 'lucide-react';
  ...
  async speak(text: string) {
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{ parts: [{ text: `Speak in a futuristic, slightly robotic but clear voice: ${text}` }] }],
        config: {
...
export default function App() {
  ...
  const handleManualSpeak = async () => {
    if (!ttsText.trim() || !ttsEngineRef.current || isSpeaking) return;
    setIsSpeaking(true);
    await ttsEngineRef.current.speak(ttsText);
    setIsSpeaking(false);
    setTtsText('');
  };

  const narrateScene = async (objects: string[], emotion: string) => {
    if (!isNarrating || !ttsEngineRef.current || ttsEngineRef.current.isPlaying) return;
    
    const now = Date.now();
    if (now - lastNarrationTime < 8000) return; // Narrate at most once every 8 seconds

    let description = "";
    if (objects.length > 0) {
      description = `Visual analysis complete. I see ${objects.join(', ')}. The calculated base emotion is ${emotion}.`;
    } else {
      description = `The field is clear. The subject appears ${emotion}.`;
    }

    setLastNarrationTime(now);
    await ttsEngineRef.current.speak(description);
  };
    ...
    if (video.readyState >= 2 && ctx) {
      ...
      try {
        ...
        if (now - lastStateUpdateTimeRef.current > 100) {
          ...
          // Trigger narration if active
          if (isNarrating) {
            narrateScene(classesArray, currentEmotion);
          }
        }
            ...
            <div className="flex flex-col gap-4 shrink-0 w-full order-2 lg:order-none pointer-events-auto">
              ...
              <div className="order-2 lg:order-1 w-full bg-black/40 backdrop-blur-md border border-white/20 p-5 shadow-[0_0_30px_rgba(0,0,0,0.8)]" title="AI-generated musical prompt based on your environment and mood">
                ...
                <div className="mt-6 pt-6 border-t border-white/10 space-y-4">
                  ...
                  <button
                    onClick={() => { playHoverSound(); setIsNarrating(!isNarrating); }}
                    className={`w-full py-2 flex items-center justify-center gap-2 text-[10px] font-bold tracking-widest border transition-all ${
                      isNarrating 
                        ? 'bg-white/20 text-white border-white shadow-[0_0_10px_rgba(255,255,255,0.3)]' 
                        : 'bg-transparent text-white/40 border-white/10'
                    }`}
                  >
                    {isNarrating ? <Volume2 className="w-3 h-3" /> : <VolumeX className="w-3 h-3" />}
                    {isNarrating ? 'AUTO_NARRATION: ON' : 'AUTO_NARRATION: OFF'}
                  </button>
                  ...
                  <div className="flex gap-2">
                    <input 
                      ...
                      placeholder="ENTER_TEXT_TO_SPEECH..."
                      className="flex-1 bg-white/5 border border-white/10 px-3 py-2 text-[10px] text-white focus:outline-none focus:border-white/40 placeholder:text-white/20"
                    />
                    <button
                      onClick={handleManualSpeak}
                      disabled={isSpeaking || !ttsText.trim()}
                      className="px-3 py-2 bg-white/10 border border-white/20 hover:bg-white/20 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                    >
                      {isSpeaking ? <Loader2 className="w-3 h-3 animate-spin" /> : <Play className="w-3 h-3 fill-current" />}
                    </button>
                  </div>
                </div>
              </div>
